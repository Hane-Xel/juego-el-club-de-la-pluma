...
public class GestorRecursos {
    ...
    public static void cargarCombate() {
        ...
        // --- AGREGAR FONDOS / ESCENARIOS AQUÍ ---
        manager.load("scenes/playa.png", Texture.class);
        manager.load("scenes/bigben.png", Texture.class);
        manager.load("scenes/dojo.png", Texture.class);
        manager.load("scenes/granmont.png", Texture.class);

        // Cargar archivos de música
        manager.load(MUSICA_PLAYA, Music.class);
        manager.load(MUSICA_BIGBEN, Music.class);
        manager.load(MUSICA_DOJO, Music.class);
        manager.load(MUSICA_GRANMONT, Music.class);
    ...
    public static Music obtenerMusica(String ruta) {
        if (manager.isLoaded(ruta, Music.class)) {
            ...
            System.err.println("Música no cargada: " + ruta);
            return null;
        }
    }
